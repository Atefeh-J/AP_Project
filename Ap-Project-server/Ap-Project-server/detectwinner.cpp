...
bool DetectWinner::isGoldenHand(const QVector<Card>& cards)
{
    if (cards.size() != 5)
        return false;
    int type=cards[0].getSuit();
    //sortCards(cards);
    for (int i = 0; i < 5; ++i) {
        if (cards[i].getSuit() != type || cards[i].getValue() != 14-i)
            return false;
    }
    return true;
}
...
bool DetectWinner::isOrderHand(const QVector<Card>& cards)
{
    ...
    //sortCards(cards);
    for (int i = 1; i < 5; ++i) {
        if (cards[i].getSuit() != type || cards[i].getValue()-cards[i-1].getValue()!=-1)
            return false;
    }
    return true;
}
...
int DetectWinner::compareSamePattern(Pattern pattern)
{
    ...
    case MSC:
        ...
        for(int i=0;i<5;i++)
        {
            if(Player1[i].getSuit()> Player2[i].getSuit())
                return 1;
            if(Player1[i].getSuit()< Player2[i].getSuit())
                return 2;
        }
